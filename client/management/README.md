...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Synqly%2FGo)

The Synqly Go library provides convenient access to the Synqly APIs from Go.
...
```go
client := client.NewClient(
    option.WithBaseURL(management.Environments.Synqly),
)
```
...
```go
response, err := client.Accounts.Create(...)
if err != nil {
...
```go
response, err := client.Accounts.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Accounts.Create(ctx, ...)
```
...
response, err := client.Accounts.Create(ctx, request, ...)
```
