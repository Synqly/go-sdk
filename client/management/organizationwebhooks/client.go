...
// Refresh the secret for a webhook.
func (c *Client) RefreshSecret(ctx context.Context, webhookId management.WebhookId, request *management.RefreshOrganizationWebhookRequest) error {
	baseURL := "https://api.synqly.com"
	if c.baseURL != "" {
		baseURL = c.baseURL
	}
	endpointURL := fmt.Sprintf(baseURL+"/"+"v1/organization/webhooks/%v/refresh_secret", webhookId)
