...
// AdditionalRestrictionStatusId Values
// Status ID. The normalized status identifier indicating the applicability of this policy restriction.

// Unknown. The status is unknown.
const AdditionalRestriction_Status_Unknown AdditionalRestrictionStatusId = 0
// Applicable. This restriction is currently applicable and being enforced.
const AdditionalRestriction_Status_Applicable AdditionalRestrictionStatusId = 1
// Inapplicable. This restriction is not applicable.
const AdditionalRestriction_Status_Inapplicable AdditionalRestrictionStatusId = 2
// EvaluationError. This restriction could not be properly evaluated due to an error.
const AdditionalRestriction_Status_EvaluationError AdditionalRestrictionStatusId = 3
// Other. The status is not mapped. See the <code>status</code> attribute, which contains a data source specific value.
const AdditionalRestriction_Status_Other AdditionalRestrictionStatusId = 99
...
// AuthenticationTokenTypeId Values
...
// Unknown. The Authentication token type is unknown.
const AuthenticationToken_Type_Unknown AuthenticationTokenTypeId = 0
// TicketGrantingTicket. Ticket Granting Ticket (TGT) for Kerberos.
const AuthenticationToken_Type_TicketGrantingTicket AuthenticationTokenTypeId = 1
// ServiceTicket. Service Ticket (ST) for Kerberos.
const AuthenticationToken_Type_ServiceTicket AuthenticationTokenTypeId = 2
// IdentityToken. Identity (ID) Token for OIDC.
const AuthenticationToken_Type_IdentityToken AuthenticationTokenTypeId = 3
// RefreshToken. Refresh Token for OIDC.
const AuthenticationToken_Type_RefreshToken AuthenticationTokenTypeId = 4
// SAMLAssertion. Authentication Assertion for SAML.
const AuthenticationToken_Type_SAMLAssertion AuthenticationTokenTypeId = 5
...
const AuthenticationToken_Type_ClientToken AuthenticationTokenTypeId = 6
...
const AuthenticationToken_Type_APIToken AuthenticationTokenTypeId = 7
// Other. The type is not mapped. See the <code>type</code> attribute, which contains a data source specific value.
const AuthenticationToken_Type_Other AuthenticationTokenTypeId = 99
...
// DataClassificationStatusId Values
// Status ID. The normalized status identifier of the classification job.

// Unknown. The status is unknown.
const DataClassification_Status_Unknown DataClassificationStatusId = 0
// Complete. The classification job completed for the evaluated resource.
const DataClassification_Status_Complete DataClassificationStatusId = 1
// Partial. The classification job partially completed for the evaluated resource.
const DataClassification_Status_Partial DataClassificationStatusId = 2
// Fail. The classification job failed for the evaluated resource.
const DataClassification_Status_Fail DataClassificationStatusId = 3
// Other. The classification job type id is not mapped.
const DataClassification_Status_Other DataClassificationStatusId = 99
...
// Dataat_Rest. The data was stored on physical or logical media and was not actively moving through the network nor was being processed. E.g., data stored in a database, PDF files in a file share, or EHR records in object storage.
...
// DataDiscovery_Classification. A tool that actively identifies and classifies sensitive data in digital media and information stores in accordance with a policy or automated functionality. E.g, Amazon Macie, Microsoft Purview.
...
// DigitalRightsManagement. A Digital Rights Management (DRM) or a dedicated Information Rights Management (IRM) are tools which can detect sensitive data and/or enforce data security policies on digital media via policy or user access rights.
...
// DeveloperSecurity. Any Developer Security tool such as an Infrastructure-as-Code (IAC) security scanner, Secrets Detection, or Secure Software Development Lifecycle (SSDLC) tool that can detect sensitive data and/or enforce data security policies. E.g., TruffleHog, GitGuardian, Git-Secrets.
...
// DataSecurityStatusId Values
// Status ID. The normalized status identifier of the classification job.

// Unknown. The status is unknown.
const DataSecurity_Status_Unknown DataSecurityStatusId = 0
// Complete. The classification job completed for the evaluated resource.
const DataSecurity_Status_Complete DataSecurityStatusId = 1
// Partial. The classification job partially completed for the evaluated resource.
const DataSecurity_Status_Partial DataSecurityStatusId = 2
// Fail. The classification job failed for the evaluated resource.
const DataSecurity_Status_Fail DataSecurityStatusId = 3
// Other. The classification job type id is not mapped.
const DataSecurity_Status_Other DataSecurityStatusId = 99
...
// NetworkZone. A managed Network Zone entity. Populate the <code>name</code> attribute with the zone name and/or the <code>uid</code> attribute with the zone ID. Additional zone information can be added to the <code>data</code> attribute.
const ManagedEntity_Type_NetworkZone ManagedEntityTypeId = 7
// Other. The type is not mapped. See the <code>type</code> attribute, which contains a data source specific value.
const ManagedEntity_Type_Other ManagedEntityTypeId = 99

// MessageContextAiRoleId Values
...
// Unknown. Unknown role type.
const MessageContext_AiRole_Unknown MessageContextAiRoleId = 0
// User. The human user initiating a request.
const MessageContext_AiRole_User MessageContextAiRoleId = 1
// Assistant. The AI model generating a response.
const MessageContext_AiRole_Assistant MessageContextAiRoleId = 2
// Tool. A callable tool or API invoked by the assistant.
const MessageContext_AiRole_Tool MessageContextAiRoleId = 3
// Agent. A software agent that performs tasks with autonomy or delegated intent.
const MessageContext_AiRole_Agent MessageContextAiRoleId = 4
// Orchestrator. The component coordinating multiple agents or tools.
const MessageContext_AiRole_Orchestrator MessageContextAiRoleId = 5
// Retriever. A component that retrieves information or context from a knowledge source (e.g., vector DB, search system, or API).
const MessageContext_AiRole_Retriever MessageContextAiRoleId = 6
// Other. Other role type not covered above.
const MessageContext_AiRole_Other MessageContextAiRoleId = 99
...
// PacketEncodingId Values
// Encoding ID. The normalized identifier of the encoding method used to represent the packet data as a string.

// Unknown. The encoding format of the packet data is not known.
const Packet_Encoding_Unknown PacketEncodingId = 0
// Base64. The packet data is encoded using Base64.
const Packet_Encoding_Base64 PacketEncodingId = 1
// Hexadecimal. The packet data is encoded as a hexadecimal string representation of the raw bytes.
const Packet_Encoding_Hexadecimal PacketEncodingId = 2
// URLEncoded. The packet data is encoded using percent-encoding (URL encoding).
const Packet_Encoding_URLEncoded PacketEncodingId = 3
// Other. The encoding method is not mapped. Refer to the <code>encoding</code> field for the original source-specific value.
const Packet_Encoding_Other PacketEncodingId = 99

// PacketFormatId Values
// Format ID. The normalized identifier of the packet capture format.

// Unknown. The packet format is unknown.
const Packet_Format_Unknown PacketFormatId = 0
// PCAP. Standard libpcap/tcpdump packet capture file format.
const Packet_Format_PCAP PacketFormatId = 1
// PCAPNG. Next-generation PCAP format that supports multiple interfaces and enhanced metadata.
const Packet_Format_PCAPNG PacketFormatId = 2
// Snoop. Solaris/Unix capture format.
const Packet_Format_Snoop PacketFormatId = 3
// ERF. Extensible Record Format used by Endace network monitoring hardware.
const Packet_Format_ERF PacketFormatId = 4
// NetMon. Microsoft Network Monitor capture file format.
const Packet_Format_NetMon PacketFormatId = 5
// 5Views. Accellent 5Views packet capture format.
const Packet_Format_5Views PacketFormatId = 6
// Other. The packet format is not mapped. Refer to the <code>format</code> field for the original source-specific value.
const Packet_Format_Other PacketFormatId = 99

// PacketSourceId Values
// Source ID. A normalized numeric identifier that specifies how the packet was obtained or generated.

// Unknown. The packet source is unknown.
const Packet_Source_Unknown PacketSourceId = 0
// Wire. The packet was captured directly from a network interface.
const Packet_Source_Wire PacketSourceId = 1
// Stream. The packet was reconstructed or derived from a stream of packets.
const Packet_Source_Stream PacketSourceId = 2
// Decoder. The packet was generated or extracted by a protocol decoder or analysis engine.
const Packet_Source_Decoder PacketSourceId = 3
// TAP. The packet was captured from a physical network Test Access Point (TAP) device used for passive monitoring.
const Packet_Source_TAP PacketSourceId = 4
// SPAN. The packet was captured from a switch Switched Port Analyzer (SPAN) or mirror port.
const Packet_Source_SPAN PacketSourceId = 5
// Endpoint. The packet was captured by a host-based agent or endpoint detection and response (EDR) sensor.
const Packet_Source_Endpoint PacketSourceId = 6
// Virtual. The packet was captured from a virtual network interface, virtual switch, or container network.
const Packet_Source_Virtual PacketSourceId = 7
// Other. The packet source is not mapped. Refer to the <code>source</code> field for the original source-specific value.
const Packet_Source_Other PacketSourceId = 99

// PeripheralDeviceTypeId Values
// Peripheral Device Type ID. The normalized peripheral device type ID.

// Unknown. The peripheral device type is unknown.
const PeripheralDevice_Type_Unknown PeripheralDeviceTypeId = 0
// ExternalStorage. The peripheral device is an external storage device.
const PeripheralDevice_Type_ExternalStorage PeripheralDeviceTypeId = 1
// Keyboard. The peripheral device is a keyboard.
const PeripheralDevice_Type_Keyboard PeripheralDeviceTypeId = 2
// Mouse. The peripheral device is a mouse.
const PeripheralDevice_Type_Mouse PeripheralDeviceTypeId = 3
// Printer. The peripheral device is a printer.
const PeripheralDevice_Type_Printer PeripheralDeviceTypeId = 4
// Monitor. The peripheral device is a monitor.
const PeripheralDevice_Type_Monitor PeripheralDeviceTypeId = 5
// Microphone. The peripheral device is a microphone.
const PeripheralDevice_Type_Microphone PeripheralDeviceTypeId = 6
// Webcam. The peripheral device is a webcam.
const PeripheralDevice_Type_Webcam PeripheralDeviceTypeId = 7
...
// Unknown. The privilege type is unknown.
const PrivilegeInfo_Type_Unknown PrivilegeInfoTypeId = 0
// Read. A read-type privilege that permits viewing or retrieving data. Examples: <code>s3:GetObject</code>, <code>Microsoft.Storage/storageAccounts/read</code>, <code>storage.objects.get</code>.
const PrivilegeInfo_Type_Read PrivilegeInfoTypeId = 1
// Write. A write-type privilege that permits creating, modifying, or deleting data. Examples: <code>s3:PutObject</code>, <code>Microsoft.Storage/storageAccounts/write</code>, <code>storage.objects.create</code>.
const PrivilegeInfo_Type_Write PrivilegeInfoTypeId = 2
// Execute. An execute-type privilege that permits running operations or invoking functions. Examples: <code>lambda:InvokeFunction</code>, <code>Microsoft.Compute/virtualMachines/start/action</code>.
const PrivilegeInfo_Type_Execute PrivilegeInfoTypeId = 3
// Other. The privilege type is not mapped. See the <code>type</code> attribute, which contains a data source specific value.
const PrivilegeInfo_Type_Other PrivilegeInfoTypeId = 99
...
// QueryEvidenceQueryTypeId Values
// Query Type ID. The normalized type of system query performed against a device or system component.

// Unknown. The query type was unknown or not specified.
const QueryEvidence_QueryType_Unknown QueryEvidenceQueryTypeId = 0
// Kernel. A query about kernel resources including system calls, shared mutex, or other kernel components.
const QueryEvidence_QueryType_Kernel QueryEvidenceQueryTypeId = 1
// File. A query about file attributes, metadata, content, hash values, or properties.
const QueryEvidence_QueryType_File QueryEvidenceQueryTypeId = 2
// Folder. A query about folder attributes, metadata, content, or structure.
const QueryEvidence_QueryType_Folder QueryEvidenceQueryTypeId = 3
// AdminGroup. A query about group membership, privileges, domain, or group properties.
const QueryEvidence_QueryType_AdminGroup QueryEvidenceQueryTypeId = 4
// Job. A query about scheduled jobs, their command lines, run states, or execution times.
const QueryEvidence_QueryType_Job QueryEvidenceQueryTypeId = 5
// Module. A query about loaded modules, their base addresses, load types, or function entry points.
const QueryEvidence_QueryType_Module QueryEvidenceQueryTypeId = 6
// NetworkConnection. A query about active network connections, boundaries, protocols, or TCP states.
const QueryEvidence_QueryType_NetworkConnection QueryEvidenceQueryTypeId = 7
// NetworkInterfaces. A query about physical or virtual network interfaces, their IP/MAC addresses, or types.
const QueryEvidence_QueryType_NetworkInterfaces QueryEvidenceQueryTypeId = 8
// PeripheralDevice. A query about attached peripheral devices, their classes, models, or vendor information.
const QueryEvidence_QueryType_PeripheralDevice QueryEvidenceQueryTypeId = 9
// Process. A query about running processes, command lines, ancestry, loaded modules, or execution context.
const QueryEvidence_QueryType_Process QueryEvidenceQueryTypeId = 10
// Service. A query about system services, their names, versions, labels, or properties.
const QueryEvidence_QueryType_Service QueryEvidenceQueryTypeId = 11
// Session. A query about authenticated user or service sessions, their creation times, or issuer details.
const QueryEvidence_QueryType_Session QueryEvidenceQueryTypeId = 12
// User. A query about user accounts, their properties, credentials, or domain information.
const QueryEvidence_QueryType_User QueryEvidenceQueryTypeId = 13
// Users. A query about multiple users belonging to an administrative group.
const QueryEvidence_QueryType_Users QueryEvidenceQueryTypeId = 14
// StartupItem. A query about startup configuration items, their run modes, start types, or current states.
const QueryEvidence_QueryType_StartupItem QueryEvidenceQueryTypeId = 15
// RegistryKey. A Windows-specific query about registry keys, their paths, security descriptors, or modification times.
const QueryEvidence_QueryType_RegistryKey QueryEvidenceQueryTypeId = 16
// RegistryValue. A Windows-specific query about registry values, their data types, content, or names.
const QueryEvidence_QueryType_RegistryValue QueryEvidenceQueryTypeId = 17
// Prefetch. A Windows-specific query about prefetch files, their run counts, last execution times, or existence.
const QueryEvidence_QueryType_Prefetch QueryEvidenceQueryTypeId = 18
// Other. The query type was not mapped to a standard category. See the query_type attribute for source-specific value.
const QueryEvidence_QueryType_Other QueryEvidenceQueryTypeId = 99

// QueryEvidenceTcpStateId Values
// TCP State ID. The state of the TCP socket for the network connection.

// Unknown. The socket state is unknown.
const QueryEvidence_TcpState_Unknown QueryEvidenceTcpStateId = 0
// ESTABLISHED. The socket has an established connection between a local application and a remote peer.
const QueryEvidence_TcpState_ESTABLISHED QueryEvidenceTcpStateId = 1
// SYN_SENT. The socket is actively trying to establish a connection to a remote peer.
const QueryEvidence_TcpState_SYN_SENT QueryEvidenceTcpStateId = 2
// SYN_RECEIVED. The socket has passively received a connection request from a remote peer.
const QueryEvidence_TcpState_SYN_RECEIVED QueryEvidenceTcpStateId = 3
...
const QueryEvidence_TcpState_FIN_WAIT_1 QueryEvidenceTcpStateId = 4
...
const QueryEvidence_TcpState_FIN_WAIT_2 QueryEvidenceTcpStateId = 5
...
const QueryEvidence_TcpState_TIME_WAIT QueryEvidenceTcpStateId = 6
// CLOSED. The socket is not in use.
const QueryEvidence_TcpState_CLOSED QueryEvidenceTcpStateId = 7
...
const QueryEvidence_TcpState_CLOSE_WAIT QueryEvidenceTcpStateId = 8
...
const QueryEvidence_TcpState_LAST_ACK QueryEvidenceTcpStateId = 9
// LISTEN. The socket is listening for incoming connections.
const QueryEvidence_TcpState_LISTEN QueryEvidenceTcpStateId = 10
...
const QueryEvidence_TcpState_CLOSING QueryEvidenceTcpStateId = 11
...
// Unknown. The run mode is unknown.
const StartupItem_RunModeIds_Unknown StartupItemRunModeIds = 0
// Interactive. The startup item interacts with the desktop.
const StartupItem_RunModeIds_Interactive StartupItemRunModeIds = 1
// OwnProcess. The startup item runs in its own process.
const StartupItem_RunModeIds_OwnProcess StartupItemRunModeIds = 2
// SharedProcess. The startup item runs in a shared process.
const StartupItem_RunModeIds_SharedProcess StartupItemRunModeIds = 3
// Other. The run mode is not mapped. See the <code>run_modes</code> attribute, which contains data source specific values.
const StartupItem_RunModeIds_Other StartupItemRunModeIds = 99

// StartupItemRunStateId Values
// Run State ID. The run state ID of the startup item.

// Unknown. The run state is unknown.
const StartupItem_RunState_Unknown StartupItemRunStateId = 0
// Stopped. The service is not running.
const StartupItem_RunState_Stopped StartupItemRunStateId = 1
// StartPending. The service is starting.
const StartupItem_RunState_StartPending StartupItemRunStateId = 2
// StopPending. The service is stopping.
const StartupItem_RunState_StopPending StartupItemRunStateId = 3
// Running. The service is running.
const StartupItem_RunState_Running StartupItemRunStateId = 4
// ContinuePending. The service is pending continue.
const StartupItem_RunState_ContinuePending StartupItemRunStateId = 5
// PausePending. The service is pending pause.
const StartupItem_RunState_PausePending StartupItemRunStateId = 6
// Paused. The service is paused.
const StartupItem_RunState_Paused StartupItemRunStateId = 7
// RestartPending. The service is pending restart.
const StartupItem_RunState_RestartPending StartupItemRunStateId = 8
// Other. The run state is not mapped. See the <code>run_state</code> attribute, which contains a data source specific value.
const StartupItem_RunState_Other StartupItemRunStateId = 99

// StartupItemStartTypeId Values
// Start Type ID. The start type ID of the startup item.

// Unknown. The start type is unknown.
const StartupItem_StartType_Unknown StartupItemStartTypeId = 0
// Auto. Service started automatically during system startup.
const StartupItem_StartType_Auto StartupItemStartTypeId = 1
// Boot. Device driver started by the system loader.
const StartupItem_StartType_Boot StartupItemStartTypeId = 2
// OnDemand. Started on demand. For example, by the Windows Service Control Manager when a process calls the <i>StartService</i> function.
const StartupItem_StartType_OnDemand StartupItemStartTypeId = 3
// Disabled. The service is disabled, and cannot be started.
const StartupItem_StartType_Disabled StartupItemStartTypeId = 4
// AllLogins. Started on all user logins.
const StartupItem_StartType_AllLogins StartupItemStartTypeId = 5
// SpecificUserLogin. Started on specific user logins.
const StartupItem_StartType_SpecificUserLogin StartupItemStartTypeId = 6
// Scheduled. Stared according to a schedule.
const StartupItem_StartType_Scheduled StartupItemStartTypeId = 7
// SystemChanged. Started when a system item, such as a file or registry key, changes.
const StartupItem_StartType_SystemChanged StartupItemStartTypeId = 8
// Other. The start type is not mapped. See the <code>start_type</code> attribute, which contains a data source specific value.
const StartupItem_StartType_Other StartupItemStartTypeId = 99

// StartupItemTypeId Values
// Type ID. The startup item type identifier.

// Unknown. The type is unknown.
const StartupItem_Type_Unknown StartupItemTypeId = 0
// KernelModeDriver. Kernel mode driver.
const StartupItem_Type_KernelModeDriver StartupItemTypeId = 1
// UserModeDriver. User mode driver.
const StartupItem_Type_UserModeDriver StartupItemTypeId = 2
// Service. A background process typically managed by the operating system, e.g., a service process on Windows or a systemd-managed daemon on Linux.
const StartupItem_Type_Service StartupItemTypeId = 3
// UserModeApplication. An application that runs in the user space.
const StartupItem_Type_UserModeApplication StartupItemTypeId = 4
// Autoload. The macOS Autoload Application.
const StartupItem_Type_Autoload StartupItemTypeId = 5
// SystemExtension. System extensions on macOS enables 3rd parties to extend the capabilities of macOS.
const StartupItem_Type_SystemExtension StartupItemTypeId = 6
// KernelExtension. Kernel extensions on macOS includes Apple provided pre-installs and 3rd party installs which enables support for specific hardware or software features not natively supported by macOS.
const StartupItem_Type_KernelExtension StartupItemTypeId = 7
// ScheduledJob_Task. A job or task that runs on a configured schedule.
const StartupItem_Type_ScheduledJob_Task StartupItemTypeId = 8
// Other. The startup item type is not mapped. See the <code>type</code> attribute, which contains data source specific values.
const StartupItem_Type_Other StartupItemTypeId = 99
...
// TicketStatusId Values
// Ticket Status ID. The normalized identifier for the ticket status.

// Unknown. The status is unknown.
const Ticket_Status_Unknown TicketStatusId = 0
// New. The ticket is new and yet to be reviewed.
const Ticket_Status_New TicketStatusId = 1
// InProgress. The ticket is actively being worked on.
const Ticket_Status_InProgress TicketStatusId = 2
// Notified. Relevant parties have been notified about the ticket.
const Ticket_Status_Notified TicketStatusId = 3
// OnHold. Work on the ticket is temporarily suspended.
const Ticket_Status_OnHold TicketStatusId = 4
// Resolved. The ticket is resolved and a solution is implemented, pending confirmation or verification from the requestor.
const Ticket_Status_Resolved TicketStatusId = 5
// Closed. The ticket has been completed and closed.
const Ticket_Status_Closed TicketStatusId = 6
// Canceled. The ticket has been canceled and will not be processed.
const Ticket_Status_Canceled TicketStatusId = 7
// Reopened. The ticket was previously closed, but has been reopened.
const Ticket_Status_Reopened TicketStatusId = 8
// Other. The status is not mapped. See the <code>status</code> attribute, which contains a data source specific value.
const Ticket_Status_Other TicketStatusId = 99
...
// Unknown_Undeclared. The UAS type is empty or not declared.
const UnmannedAerialSystem_Type_Unknown_Undeclared UnmannedAerialSystemTypeId = 0
// Airplane
const UnmannedAerialSystem_Type_Airplane UnmannedAerialSystemTypeId = 1
// Helicopter. Can also be a Multi-rotor Unmanned Aircraft (e.g., Quad-copter).
const UnmannedAerialSystem_Type_Helicopter UnmannedAerialSystemTypeId = 2
// Gyroplane
const UnmannedAerialSystem_Type_Gyroplane UnmannedAerialSystemTypeId = 3
// HybridLift. Fixed wing aircraft that can take off vertically.
const UnmannedAerialSystem_Type_HybridLift UnmannedAerialSystemTypeId = 4
// Ornithopter
const UnmannedAerialSystem_Type_Ornithopter UnmannedAerialSystemTypeId = 5
// Glider
const UnmannedAerialSystem_Type_Glider UnmannedAerialSystemTypeId = 6
// Kite
const UnmannedAerialSystem_Type_Kite UnmannedAerialSystemTypeId = 7
// FreeBalloon
const UnmannedAerialSystem_Type_FreeBalloon UnmannedAerialSystemTypeId = 8
// CaptiveBalloon
const UnmannedAerialSystem_Type_CaptiveBalloon UnmannedAerialSystemTypeId = 9
// Airship. E.g., a blimp.
const UnmannedAerialSystem_Type_Airship UnmannedAerialSystemTypeId = 10
// FreeFall_Parachute. Parachutes, or objects without any power or propulsion mechanism.
const UnmannedAerialSystem_Type_FreeFall_Parachute UnmannedAerialSystemTypeId = 11
// Rocket
const UnmannedAerialSystem_Type_Rocket UnmannedAerialSystemTypeId = 12
// TetheredPoweredAircraft
const UnmannedAerialSystem_Type_TetheredPoweredAircraft UnmannedAerialSystemTypeId = 13
// GroundObstacle
const UnmannedAerialSystem_Type_GroundObstacle UnmannedAerialSystemTypeId = 14
// Other. The type is not mapped. See the <code>type</code> attribute, which contains a data source specific value.
const UnmannedAerialSystem_Type_Other UnmannedAerialSystemTypeId = 99

// UnmannedSystemOperatingAreaTypeId Values
// Type ID. The operating area type identifier.

// Unknown_Undeclared. The UA type is empty or not declared.
const UnmannedSystemOperatingArea_Type_Unknown_Undeclared UnmannedSystemOperatingAreaTypeId = 0
// TakeoffLocation
const UnmannedSystemOperatingArea_Type_TakeoffLocation UnmannedSystemOperatingAreaTypeId = 1
// FixedLocation
const UnmannedSystemOperatingArea_Type_FixedLocation UnmannedSystemOperatingAreaTypeId = 2
// DynamicLocation
const UnmannedSystemOperatingArea_Type_DynamicLocation UnmannedSystemOperatingAreaTypeId = 3
// Other. The type is not mapped. See the <code>type</code> attribute, which contains a data source specific value.
const UnmannedSystemOperatingArea_Type_Other UnmannedSystemOperatingAreaTypeId = 99
