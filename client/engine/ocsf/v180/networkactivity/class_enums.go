...
// Unknown. The observation point is unknown.
const ObservationPoint_Unknown ObservationPointId = 0
// Source. The source network endpoint is the observation point.
const ObservationPoint_Source ObservationPointId = 1
// Destination. The destination network endpoint is the observation point.
const ObservationPoint_Destination ObservationPointId = 2
// Neither. Neither the source nor destination network endpoint is the observation point. Refer to the <code>network_observation_point</code> attribute for details.
const ObservationPoint_Neither ObservationPointId = 3
// Both. Both the source and destination network endpoint are the observation point. This typically occurs in localhost or internal communications where the source and destination are the same endpoint, often resulting in a <code>connection_info.direction</code> of <code>Local</code>.
const ObservationPoint_Both ObservationPointId = 4
// Other. The observation point is not mapped. See the <code>observation_point</code> attribute for a data source specific value.
const ObservationPoint_Other ObservationPointId = 99
